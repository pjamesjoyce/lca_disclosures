...
class LcoptDisclosureExporter(BaseExporter):
    ...
    def _prepare_efn(self):
        ...
        if hasattr(self, 'filename'):
            ...
            if self.parameter_set is None:
                efn = '{}_unspecified.json'.format(self.model.name.replace(" ", "_"))
            else:
    ...
    def _prepare_disclosure(self):
        ...
        fu = [(i, x) for i, x in enumerate(self.model.names) if list(matrix.sum(axis=1))[i] == 0 and list(matrix.sum(axis=0))[i] != 0]
        unused = [(i, x) for i, x in enumerate(self.model.names) if list(matrix.sum(axis=1))[i] == 0 and list(matrix.sum(axis=0))[i] == 0]
        ...
        technosphere_links = [self.model.database['items'][self.model.get_exchange(x[1])].get('ext_link',(None, '{}'.format(x[1]))) for x in background if self.model.database['items'][self.model.get_exchange(x[1])]['lcopt_type'] == "input"]
        biosphere_links = [self.model.database['items'][self.model.get_exchange(x[1])]['ext_link'] for x in background if self.model.database['items'][self.model.get_exchange(x[1])]['lcopt_type'] == "biosphere"]
