...
class DisclosureExporter(BaseExporter):
    ...
    def _prepare_disclosure(self):
        ...
        if self.fu is not None and self.fu in foreground:
            fu_list = [self.fu]
            foreground = fu_list + [x for x in foreground if x not in fu_list] # set fu to be the first item in the foreground matrix
            ...
            fu_list = [foreground[i] for i, x in enumerate(foreground) if list(temp_matrix.sum(axis=1))[i] == 0 and list(temp_matrix.sum(axis=0))[i] != 0]
            foreground = fu_list + [x for x in foreground if x not in fu_list] # set fu to be the first item in the foreground matrix
        ...
        biosphere_names = [
                            {
                            'index':i, 
                            'name':"{}, {}, {}".format(biosphere_info[i]['name'], biosphere_info[i]['type'],  ",".join(biosphere_info[i]['categories'])),
